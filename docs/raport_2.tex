...
Mimo zerowych wartości metryk ROUGE, proces uczenia przebiegał poprawnie z punktu widzenia optymalizacji funkcja straty systematycznie malała, a model generował sekwencje o niezerowej długości\footnote{Wykres przebiegu treningu dostępny pod adresem: \url{https://wandb.ai/tombik-warsaw-university-of-technology/nlp-project-peft/runs/y89fgtys/panel/gyjco2lrh?nw=nwusertombik}}. Sugeruje to, że model nie uległ degeneracji. Zaobserwowana rozbieżność między spadkiem Loss a zerowym ROUGE wskazuje na potencjalne problemy z ewaluacją. Prawdopodobnym wyjaśnieniem jest ograniczona skuteczność standardowej implementacji metryki ROUGE dla języków o alfabecie innym niż łaciński (problem z tokenizacją) lub specyfika morfologiczna języka nepalskiego.
...
% Dopisanie sekcji LowData (mT5-es)

\section{Podsumowanie}

Przeprowadzone eksperymenty pozwoliły na porównanie efektywności różnych metod PEFT w dwóch odmiennych zadaniach NLP.

\begin{itemize}
    \item \textbf{Zadanie klasyfikacji (BERT):} Najlepszym kompromisem okazała się metoda \textbf{LoRA} oraz \textbf{Adaptery (Houlsby)}. Obie techniki pozwoliły uzyskać wyniki niemal identyczne z pełnym fine-tuningiem (Accuracy $>99\%$) przy skróceniu czasu treningu. Prefix Tuning okazał się w tym przypadku niewystarczający.
    
    \item \textbf{Zadanie streszczania (mT5):} Tutaj również \textbf{Adaptery (Pfeiffer)} oraz \textbf{LoRA} wykazały się wysoką skutecznością. Co ciekawe, Adaptery pozwoliły uzyskać wyniki lepsze od baseline'u, co może wynikać z mniejszej podatności na przeuczenie przy ograniczonej liczbie parametrów. Prefix Tuning ponownie odstawał jakościowo od reszty stawek.
\end{itemize}

Warto odnotować, że planowane eksperymenty z metodami PEFT na ograniczonym zbiorze hiszpańskim (low-data) nie zostały w pełni zrealizowane z powodu wyczerpania limitów obliczeniowych (Google Colab). Jednakże wyniki uzyskane przy pełnym fine-tuningu na zredukowanym zbiorze pozwalają przypuszczać, że metody PEFT również poradziłyby sobie dobrze w tym scenariuszu, nie tracąc znacząco na jakości.

Ostatecznie, \textbf{LoRA} jawi się jako najbardziej uniwersalna metoda, oferująca świetny stosunek jakości do czasu treningu w obu typach zadań. \textbf{Adaptery} są silną konkurencją, szczególnie w zadaniach generatywnych, gdzie mogą przewyższać jakością inne podejścia kosztem nieco dłuższego czasu uczenia.
